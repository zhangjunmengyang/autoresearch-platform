    ...
    <div className="grid-two">
      <Panel title="就绪检查">
        ...
        >
          ...
          <TextInput label="轮次标识" value={roundId} onChange={setRoundId} placeholder="可选：round_" />
          <SelectInput label="检查阶段" value={stage} onChange={setStage} options={stageOptions} />
          <Button type="submit" disabled={!query}>检查</Button>
        </form>
        <p className="panel-copy">
          就绪检查只读取上下文、轮次追踪包、实验、artifact、证据和决策引用，不执行实验、不调用模型、不创建 workflow。
        </p>
        ...
        {result ? (
          <div className="record-list">
            <article className="record-row">
              <div>
                <div className="record-title">状态 {result.state}</div>
                <div className="record-meta">阶段 {result.stage} · 查询 {result.query}</div>
              </div>
              ...
              <div>
                ...
                <div className="record-meta">
                  {result.blocking_gaps.length
                    ? result.blocking_gaps.map((gap) => `${gap.title}:${gap.detail}`).join('，')
                    : '无'}
                </div>
              </div>
            </article>
            ...
            <article className="record-row">
              <div>
                ...
                <div className="record-meta">
                  {result.checks.map((check) => `${check.title}:${check.status}`).join('，')}
                </div>
              </div>
            </article>
              ...
              <div>
                ...
                <div className="record-meta">
                  {result.round_trace?.trace?.unresolved_refs?.length
                    ? result.round_trace.trace.unresolved_refs.map((ref) => `${ref.field}:${ref.type}:${ref.id}:${ref.reason}`).join('，')
                    : '无'}
                </div>
              </div>
            </article>
              ...
              <div>
                ...
                <div className="record-meta">
                  {result.recommended_next_actions.length
                    ? result.recommended_next_actions.map((action) => `${action.action || '动作'} -> ${action.endpoint || '未记录 endpoint'}`).join('，')
                    : '暂无'}
                </div>
              </div>
            </article>
